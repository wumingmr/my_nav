...
2. 打开导航页 → 设置 →「坚果云备份」:
   ...
   - 点「☁ 备份到坚果云」:书签+分类数据打包上传到 `dav.jianguoyun.com/dav/my_nav_backup/`,文件名带时间戳(如 `nav-20260819-103045.json`),每次备份都是独立文件
   - 点「☁ 从坚果云恢复」:列出云端全部备份(文件名/时间/大小),**选择其中一个**恢复并覆盖当前数据(有确认提示)
...
- 备份文件存于 `my_nav_backup/` 目录,每次备份生成新文件,不会覆盖;坚果云免费版自带 30 天文件历史版本,误操作可去坚果云网页找回
